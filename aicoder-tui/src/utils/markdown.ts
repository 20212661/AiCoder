...
// NOTE: This module is not imported by any other module (orphaned migration code).
// The following were stubbed out from non-existent imports:
const BLOCKQUOTE_BAR = '│'
type CliHighlight = {
  highlight(code: string, lang?: string): string
  supportsLanguage(lang: string): boolean
}
const createHyperlink = (url: string, text?: string) => supportsHyperlinks() ? `\x1b]8;;${url}\x1b\\${text ?? url}\x1b]8;;\x1b\\` : `${text ?? url} (${url})`
const stripPromptXMLTags = (s: string) => s
  ...
  switch (token.type) {
    ...
    case 'link': {
      ...
      if (plainLinkText && plainLinkText !== token.href) {
        return createHyperlink(token.href, linkText) as string      }
      // When the display text matches the URL (or is empty), just show the URL
      return createHyperlink(token.href, token.href)
    }
    case 'list': {
      return token.items
