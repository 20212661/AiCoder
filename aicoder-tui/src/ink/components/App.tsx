...
export default class App extends PureComponent<Props, State> {
  ...
  handleSetRawMode = (isEnabled: boolean): void => {
    ...
    if (isEnabled) {
      ...
      if (this.rawModeEnabledCount === 0) {
        ...
        // On Windows Terminal (WT_SESSION), 'readable' event often doesn't fire
        // when the Node.js process is launched inside a wrapper like tsx watch.
        // The 'data' event is more reliable on Windows, ensuring keyboard input
        // is received even when 'readable' stalls.
        if (process.env.WT_SESSION || process.platform === 'win32') {
          stdin.addListener('data', this.handleDataFallback);
        } else {
    ...
    if (--this.rawModeEnabledCount === 0) {
      ...
      stdin.removeListener('data', this.handleDataFallback);
      stdin.unref();
    }
  };
  ...
  /** Fallback handler for 'data' events on Windows where 'readable' may not fire. */
  handleDataFallback = (chunk: Buffer): void => {
